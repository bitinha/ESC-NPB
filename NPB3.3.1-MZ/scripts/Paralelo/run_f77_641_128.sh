...
for i in {1..10}
do
    time mpirun -np 64 --map-by ppr:1:core -mca btl self,sm,tcp bin/bt-mz.W.64
done
echo '------------Class A------------'

for i in {1..10}
do
    time mpirun -np 64 --map-by ppr:1:core -mca btl self,sm,tcp bin/bt-mz.A.64
done
echo '------------Class B------------'

for i in {1..10}
do
    time mpirun -np 64 --map-by ppr:1:core -mca btl self,sm,tcp bin/bt-mz.B.64
done
echo '------------Class C------------'


time mpirun -np 64 --map-by ppr:1:core -mca btl self,sm,tcp bin/bt-mz.C.64
